    ...
    <section className="space-y-5">
      <div className="grid gap-4 rounded-lg border bg-muted/20 p-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="space-y-2">
          ...
          <Select
            value={method}
            onValueChange={(value) => setMethod(value as PaymentMethod | "all")}
          >
            <SelectTrigger className="min-h-12">
              <SelectValue />
            </SelectTrigger>
      ...
      {journal ? (
        <div className="grid gap-px overflow-hidden rounded-lg border bg-border sm:grid-cols-4">
          <div className="bg-blue-700 p-4 text-blue-50">
            <p className="text-xs text-blue-100">Total confirmé</p>
            <p className="mt-1 text-xl font-semibold tabular-nums">
              {formatFcfa(journal.totals.grandTotal)}
            </p>
          </div>
          {methods.map((item) => (
            <div key={item.value} className="bg-card p-4">
              <p className="text-xs text-muted-foreground">{item.label}</p>
              <p className="mt-1 text-xl font-semibold tabular-nums">
                {formatFcfa(journal.totals[item.value])}
              </p>
            </div>
          ))}
        </div>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
        ...
        <Button
          className="min-h-12"
          disabled={pdf.isRunning}
          onClick={() => void pdf.launch(() => exportCashJournalPdf(filter))}
        >
          {pdf.isRunning ? (
            <Loader2 className="mr-2 h-4 w-4 animate-spin" />
          ) : (
      ...
      {journalQuery.isError ? (
        <QueryErrorState
          message="Impossible de charger le journal de caisse."
      ...
      {journal && journal.entries.length === 0 ? (
        <EmptyState
          ...
          message="Modifiez les filtres ou élargissez la période."
        />
      ) : null}
      {journal && journal.entries.length > 0 ? (
        <div className="space-y-4">
                  ...
                  <TableRow key={entry.id}>
                    ...
                    <TableCell>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {entry.studentMatricule ?? "Sans matricule"}
                      </p>
                    </TableCell>
                    <TableCell>{entry.className}</TableCell>
                    <TableCell>
                      {
                        methods.find((item) => item.value === entry.method)
                          ?.label
                      }
                    </TableCell>
                    <TableCell>
                      {entry.providerReference ??
                        entry.schoolReceiptReference ??
                        "—"}
                    </TableCell>
          ...
          <div className="flex flex-col gap-3 border-t pt-4 text-sm sm:flex-row sm:items-center sm:justify-between">
            <p className="text-muted-foreground">
              {pageStart}–{pageEnd} sur {totalEntries} écriture
              {totalEntries > 1 ? "s" : ""}
            </p>
            <div className="flex items-center gap-2">
              <Button
                ...
                size="sm"
                className="min-h-12"
                disabled={currentPage === 1 || journalQuery.isFetching}
              ...
              <span
                ...
                aria-live="polite"
              >
                Page {journal.pagination.page} / {totalPages}
              </span>
              <Button
                type="button"
                variant="outline"
                ...
                className="min-h-12"
                disabled={
                  journal.pagination.page === totalPages ||
