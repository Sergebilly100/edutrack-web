...
test("setup teacher auth state", async ({ page }) => {
  await mkdir(AUTH_DIR, { recursive: true })
  const storagePath = path.join(AUTH_DIR, "teacher.json")

  await loginAndSaveState(
    {
      identifier: process.env.E2E_TEACHER_IDENTIFIER ?? "kouadio.nguessan@sainte-marie.ci",
