...
test("setup staff auth state", async ({ page }) => {
  await mkdir(AUTH_DIR, { recursive: true })
  const storagePath = path.join(AUTH_DIR, "staff.json")

  await loginAndSaveState(
    {
      identifier: process.env.E2E_STAFF_IDENTIFIER ?? "secretariat@sainte-marie.ci",
      password: process.env.E2E_STAFF_PASSWORD ?? "Test1234!",
