...
const loginAndSaveState = async (input: LoginInput, page: Page) => {
...
test("setup director auth state", async ({ page }) => {
  await mkdir(AUTH_DIR, { recursive: true })
  const storagePath = path.join(AUTH_DIR, "director.json")

  await loginAndSaveState(
    {
      identifier: process.env.E2E_DIRECTOR_IDENTIFIER ?? "directeur@sainte-marie.ci",
      password: process.env.E2E_DIRECTOR_PASSWORD ?? "Test1234!",
      schemaName: process.env.E2E_SCHEMA_NAME ?? "school_sainte_marie",
